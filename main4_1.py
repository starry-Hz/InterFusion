...
def calculate_ips(
    true_anomalies: Dict[int, Set[int]],  # 各异常段及其真实异常指标 {段ID: {指标1, 指标2}}
    predicted_anomalies: Dict[int, Set[int]],  # 各异常段及其预测异常指标
    segment_lengths: Dict[int, int]  # 各异常段的长度N_{Φ_a}
) -> float:
    """
    严格遵循图片定义的IPS计算
    公式:
        IPS = Σ_{a=1}^A [ w_a * (|G_{Φ_a} ∩ I_{Φ_a}| / |G_{Φ_a}|) ]
        其中 w_a = N_{Φ_a} / Σ_{a=1}^A N_{Φ_a}
    """
    ...
    ips_score = 0.0
    for seg_id in true_anomalies:
        G = true_anomalies[seg_id]  # 真实异常指标集合 G_{Φ_a}
        I = predicted_anomalies.get(seg_id, set())  # 预测异常指标集合 I_{Φ_a}
        N = segment_lengths.get(seg_id, 0)  # 当前段长度 N_{Φ_a}
        
        # 计算当前段的解释精确率
        if len(G) > 0:
            precision = len(G & I) / len(G)
        else:
            precision = 0.0
        
        # 计算权重
        weight = N / total_length
        
        # 累加到总分
        ips_score += weight * precision
...
def evaluate_single_dataset(
...
) -> Tuple[float, float, float, float, Dict[int, Set[int]], Dict[int, Set[int]], Dict[int, int]]:
    """
    评估单个数据集的性能（完整实现，严格遵循图片中的IPS公式）
    
    Args:
        train_data: 训练数据 [T_train, M]
        test_data: 测试数据 [T_test, M] 
        test_labels: 测试标签 [T_test]
        gt_txt_path: 标注文件路径
        config: 实验配置
        score_mode: 评分模式
        threshold: 异常分数阈值
        neighbor_param: 邻居选择参数
        
    Returns:
        tuple: (precision, recall, f1_score, ips_score, 
                true_anomalies_dict, predicted_anomalies_dict, segment_lengths_dict)
    """
    # ==================== 1. 初始化数据结构 ====================
    true_anomalies_dict = {}    # {段ID: 真实异常指标集合 G_Φa}
    predicted_anomalies_dict = {}  # {段ID: 预测异常指标集合 I_Φa} 
    segment_lengths_dict = {}   # {段ID: 异常段长度 N_Φa}
    
    total_TP = total_FP = total_FN = 0  # 传统指标计算用
    ...
    with open(gt_txt_path, 'r') as f:
        for seg_id, line in enumerate(f):
            ...
            sensors = set(map(int, sensors_str.split(',')))
            ...
            # 存储真实异常和段长度
            true_anomalies_dict[seg_id] = sensors
            segment_lengths_dict[seg_id] = end - start + 1
    ...
    # ==================== 5. 异常检测主循环 ====================
    for seg_id, segment in enumerate(gt_segments):
        start, end = segment['start'], segment['end']
        gt_sensors = segment['sensors']
        
        # 初始化当前段的预测结果
        predicted_nodes = set()
        abnormal_scores = defaultdict(float)
        ...
        for i in range(start, end - config.window_size + 2):
            ...
            for j in range(num_sensors):
                for k in topk_neighbors[j]:
                    key = (min(j, k), max(j, k))
                    corr_val = window_corr[j, k]
                    
                    if not np.isnan(corr_val):
                        deviation = calculate_abnormal_score(
                            corr_val, edge_stats, key, score_mode
                        )
                        if deviation > 0:
                            abnormal_scores[j] += deviation
                            abnormal_scores[k] += deviation
        
        # 应用阈值筛选异常节点
        predicted_nodes = {
            node + 1 for node, score in abnormal_scores.items() 
            if score >= threshold
        }
        predicted_anomalies_dict[seg_id] = predicted_nodes
        
        # 计算传统指标
        TP = len(predicted_nodes & gt_sensors)
        FP = len(predicted_nodes - gt_sensors)
        FN = len(gt_sensors - predicted_nodes)
    ...
    # IPS指标（严格遵循图片公式）
    ips_score = 0.0
    total_length = sum(segment_lengths_dict.values())
    
    if total_length > 0:
        for seg_id in true_anomalies_dict:
            G = true_anomalies_dict[seg_id]
            I = predicted_anomalies_dict.get(seg_id, set())
            N = segment_lengths_dict[seg_id]
            
            if len(G) > 0:
                ips_score += (N / total_length) * (len(G & I) / len(G))
    ...
    return {
        ...
        'FP': total_FP,
        'FN': total_FN,
        'true_anomalies': true_anomalies_dict,
        'predicted_anomalies': predicted_anomalies_dict,
        'segment_lengths': segment_lengths_dict
    }
...
def grid_search_worker(
    ...
    eval_results = evaluate_single_dataset(
    ...
    return {
        ...
        'ips_score': eval_results['ips_score'],
        'true_anomalies': eval_results['true_anomalies'],
        'predicted_anomalies': eval_results['predicted_anomalies'],  # 确保包含这个键
        'segment_lengths': eval_results['segment_lengths'],
        'TP': eval_results['TP'],
        'FP': eval_results['FP'],
        'FN': eval_results['FN']
    }
...
def summarize_best_results(
    ...
    # 检查必要的列是否存在
    required_columns = ['TP', 'FP', 'FN', 'true_anomalies', 'predicted_anomalies', 'segment_lengths']
    for col in required_columns:
        if col not in best_per_dataset.columns:
            logger.error(f"Missing required column '{col}' in results DataFrame")
            return pd.DataFrame(), {
                'overall_precision': 0.0,
                'overall_recall': 0.0,
                'overall_f1_score': 0.0,
    ...
    # ==================== 3. 计算整体传统指标 ====================
    total_TP = best_per_dataset['TP'].sum()
    total_FP = best_per_dataset['FP'].sum()
    total_FN = best_per_dataset['FN'].sum()
    ...
    for _, row in best_per_dataset.iterrows():
        # 确保这些字段是字典类型
        true_anomalies = row['true_anomalies'] if isinstance(row['true_anomalies'], dict) else {}
        predicted_anomalies = row['predicted_anomalies'] if isinstance(row['predicted_anomalies'], dict) else {}
        segment_lengths = row['segment_lengths'] if isinstance(row['segment_lengths'], dict) else {}

        for seg_id, nodes in true_anomalies.items():
            merged_true[seg_id_offset + seg_id] = nodes
            merged_lengths[seg_id_offset + seg_id] = segment_lengths.get(seg_id, 0)
        
        for seg_id, nodes in predicted_anomalies.items():
            merged_pred[seg_id_offset + seg_id] = nodes
        
        seg_id_offset += 1000

    # 计算整体IPS
    overall_ips = 0.0
    total_length = sum(merged_lengths.values())

    if total_length > 0:
        for seg_id in merged_true:
            G = merged_true[seg_id]
            I = merged_pred.get(seg_id, set())
            N = merged_lengths.get(seg_id, 0)

            if len(G) > 0:
                overall_ips += (N / total_length) * (len(G & I) / len(G))

    # ==================== 5. 日志输出 ====================
    logger.info("\n===== Best Results per Dataset =====\n" + 
               best_per_dataset[['dataset', 'precision', 'recall', 'f1_score', 'ips_score']].to_string(index=False))
    
    logger.info(f"\n===== Overall Metrics =====\n"
               f"Precision: {overall_precision:.4f}  "
               f"Recall: {overall_recall:.4f}  "
               f"F1: {overall_f1:.4f}  "
               f"IPS: {overall_ips:.4f}\n"
...
if __name__ == "__main__":
    ...
    # 设置邻居选择方式为相关系数阈值
    neighbor_selection = 'corr_threshold'
