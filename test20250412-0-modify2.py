...
#                               window_size=5, score_threshold=3.0, top_k=5, score_mode='deviation'):
...
#     def detect_anomalous_nodes(start, end, score_mode):
...
#                         elif score_mode == 'mean_ratio':
#                             deviation = abs(val / stats['mean']) if stats['mean'] != 0 else 0
#                         elif score_mode == 'range_ratio':
#                             deviation = abs(val / stats['range']) if stats['range'] != 0 else 0
#                         elif score_mode == 'value_times_range':
#                             deviation = abs(val * stats['range'])
#                         elif score_mode == 'robust_zscore':
#                             deviation = np.abs(val - stats['median']) / (stats['mad'] + 1e-8)
#                         else:
#                             deviation = max(abs(val - low), abs(val - high))
...
def evaluate_omi_dataset_topk(train_data, test_data, test_labels, gt_txt_path,
    ...
    for segment in gt_segments:
        ...
        predicted = detect_anomalous_nodes(start, end, score_mode)
...
def grid_search_topk(train_data, test_data, test_labels, gt_txt_path,
                     topk_list, threshold_list, score_mode_list, dataset_name=None):
    results = []
        ...
        for top_k in topk_list:
            for threshold in threshold_list:
                precision, recall, f1 = evaluate_omi_dataset_topk(
                    ...
                    window_size=5, score_threshold=threshold, top_k=top_k, score_mode=score_mode
                )
                logger.info(f"[{dataset_name}] mode={score_mode}, top_k={top_k}, threshold={threshold:.1f}, "
...
def run_full_grid_search_all_datasets(data_folder, label_folder,
    ...
    for txt_file in txt_files:
        ...
        df = grid_search_topk(train_data, test_data, test_labels, gt_txt_path,
                              topk_list, threshold_list, score_mode_list, dataset_name=txt_file)
        df['dataset'] = txt_file
