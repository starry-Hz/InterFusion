...
class ExperimentConfig:
    ...
    log_dir: str = "./log"
    visual_dir: str = f"./visual/{use_topk}"
    num_workers: int = 4
...
def run_full_grid_search(config: ExperimentConfig, logger: logging.Logger) -> pd.DataFrame:
    ...
    for txt_file in txt_files:
        ...
        if hasattr(train_data, 'values'):  # 检查是否有values属性
...
def visualize_anomaly_graph(
    ...
    for j in range(num_sensors):
        for k in topk_neighbors[j]:
            ...
            if np.isnan(val):
                continue
            low, high = edge_stats[key]['percentiles']
...
def run_visualization(
    ...
    for _, row in best_results.iterrows():
        ...
        train_data = pd.read_pickle(train_path).values
        test_data = pd.read_pickle(test_path).values
        test_labels = pd.read_pickle(label_path).values
