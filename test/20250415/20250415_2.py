...
@dataclass  # 定义实验配置,使用 dataclass 以便于管理参数
class ExperimentConfig:
    dataset: str = 'omi'  # 默认数据集
    window_size: int = 5  # 默认窗口大小
    score_thresholds: npt.NDArray = field(default_factory=lambda: np.arange(1.0, 7.1, 0.5))  # 默认阈值范围
    topk_range: range = field(default_factory=lambda: range(1, 10))  # 默认 top_k 范围
    score_modes: List[str] = field(default_factory=lambda: ['mean_ratio'])  # 默认评分模式
    use_topk: bool = True  # 默认是否使用 top_k
    data_dir: str = "./data/processed"  # 默认数据目录
    label_dir: str = "./data/interpretation_label"  # 默认标签目录
    ...
    def __post_init__(self):
        ...
        self.visual_dir = f"./visual/use_topk{self.use_topk}/{self.score_modes[0]}"
# 其他评分模式（可选）：
...
if __name__ == "__main__":
    ...
    for use_topk in [True, False]:
        for score_mode in [['deviation'], ['mean_ratio'], ['range_ratio'], ['value_times_range'], ['robust_zscore']]:
            print(f"\n{'='*50}")
