...
def visualize_anomaly_graph(test_data, start, end, edge_thresholds, topk_neighbors,
    ...
    for j in range(num_sensors):
        for k in topk_neighbors[j]:
            ...
            G.add_edge(j + 1, k + 1, color='red' if is_abnormal else 'dimgray')  # 🔴 or ⚫
    ...
    for node in G.nodes():
        if node in ground_truth_nodes and node in detected_nodes:
            node_colors.append('purple')       # 🟪 正确检测（紫色）
        elif node in ground_truth_nodes:
            node_colors.append('blue')         # 🔵 实际异常（蓝色）
        elif node in detected_nodes:
            node_colors.append('orange')       # 🟠 检测异常（橙色）
        else:
            node_colors.append('lightgray')    # ⚪ 正常节点（浅灰色）
...
def evaluate_omi_dataset_topk(train_data, test_data, test_labels, gt_txt_path,
    ...
    def detect_anomalous_nodes(start, end):
        '''
        score_mode='deviation' 为原始模式（即超出上下界的偏差）
        score_mode='mean_ratio' 表示使用 value / mean
        score_mode='range_ratio' 表示使用 value / range
        score_mode='value_times_range' 表示使用 value * range
        '''
                ...
                for k in neighbors:
                    ...
                    if val < low or val > high:
                        if score_mode == 'deviation':
                            ...
                            raise ValueError(f"未知得分模式：{score_mode}")
...
if __name__ == "__main__":
    ...
    for txt_file in txt_files:
        ...
        precision, recall, f1 = evaluate_omi_dataset_topk(
            ...
            top_k=5,  # 如果 use_topk=True
            save_fig_dir=save_fig_dir,
            score_mode='value_times_range',  # 可选: deviation, mean_ratio, range_ratio, value_times_range
            use_topk=False  # 遍历所有邻居
        )
