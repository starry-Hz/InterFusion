...
class SensorRelationAnalyzer:
    ...
    def build_relation_graphs(self, data, labels):
        print(f"\n[INFO] 输入数据形状: {data.shape}, 传感器数量: {data.shape[1]}")
        ...
        print(f"[INFO] 正常窗口数: {len(normal_windows)}, 异常窗口数: {len(anomaly_windows)}")

        self.normal_graph = self._build_graph(normal_windows)
        self.anomaly_graph = self._build_graph(anomaly_windows)

        print(f"[INFO] 正常图: {len(self.normal_graph.nodes())} 节点, {len(self.normal_graph.edges())} 边")
        print(f"[INFO] 异常图: {len(self.anomaly_graph.nodes())} 节点, {len(self.anomaly_graph.edges())} 边")
        ...
        for i in range(len(data) - self.window_size + 1):
            if labels[i + self.window_size - 1] == target_label:
                ...
                if win.shape[1] == expected_dim:
                    ...
                    print(f"[WARN] 跳过维度异常窗口: 第{i}个窗口 shape={win.shape}")
        if not windows:
            print("[WARN] 没有符合条件的窗口被提取")

        windows = np.array(windows)
        print(f"[INFO] 提取窗口数组形状: {windows.shape}")
        return windows

    def _build_graph(self, windows):
        if len(windows) == 0:
            print("[WARN] 无窗口数据，返回空图")
            return nx.Graph()

        n_sensors = windows.shape[2]
        print(f"[INFO] 构建图中，传感器数量: {n_sensors}")
        ...
        for i in range(n_sensors):
            for j in range(i + 1, n_sensors):
                try:
                    ...
                    print(f"[ERROR] 处理传感器对({i},{j})时出错: {str(e)}")
...
class DeepWalkDetector:
    ...
    def _train_deepwalk(self, graph, graph_type):
        if len(graph.edges()) == 0:
            print(f"[WARN] {graph_type}图没有边，跳过训练")
            return None

        print(f"[INFO] 训练{graph_type}模型，节点数: {len(graph.nodes())}, 边数: {len(graph.edges())}")
    ...
    def _detect_window_anomalies(self, window, threshold):
        ...
        for i in range(n_sensors):
            for j in range(i+1, n_sensors):
                try:
                    ...
                    print(f"[ERROR] 窗口检测中处理传感器对({i},{j})出错: {str(e)}")
        ...
        for sensor in range(n_sensors):
            try:
                ...
                print(f"[ERROR] 计算传感器{sensor}异常得分时出错: {str(e)}")
...
def load_and_preprocess(txt_file):
    ...
    print(f"[INFO] 原始数据形状: {data.shape}, 标签形状: {labels.shape}")

    train_size = int(len(data) * 0.7)
    train_data, test_data = data[:train_size], data[train_size:]
    train_labels, test_labels = labels[:train_size], labels[train_size:]

    print(f"[INFO] 训练集: {train_data.shape}, 测试集: {test_data.shape}")
...
def evaluate(ground_truth, predictions, test_data):
    ...
    print(f"[RESULT] 召回率: {recall * 100:.1f}% | 精确率: {precision * 100:.1f}%")
    print(f"[RESULT] TP: {stats['true_positives']}, FP: {stats['false_positives']}, Total Detected: {stats['detected']}, Expected: {stats['expected']}")
    return stats
