...
class SensorRelationAnalyzer:
    ...
    def _extract_windows(self, data, labels, target_label=0):
        """提取指定标签的滑动窗口，确保每个窗口维度一致"""
        windows = []
        expected_dim = data.shape[1]  # 应该是 19
        ...
        for i in range(len(data) - self.window_size + 1):
            if labels[i + self.window_size - 1] == target_label:
                ...
                if win.shape[1] == expected_dim:
                    ...
                    print(f"跳过维度异常窗口: 期望{expected_dim}, 实际{win.shape[1]}")
    ...
    def _build_graph(self, windows):
        ...
        if len(windows) == 0:
            return nx.Graph()
        print(f"windows形状: {windows.shape}, 传感器数量: {windows.shape[1]}")
        n_sensors = windows.shape[2]  # 实际传感器数量
        print(f"构建图中，传感器数量: {n_sensors}")
        
        G = nx.Graph()
        G.add_nodes_from(range(n_sensors))  # 只添加实际存在的传感器节点
